...
int FactorGraph::RunPSDD(double lower_bound,
  ...
  for (t = 0; t < psdd_max_iterations_; ++t) {
    ...
    for (int j = 0; j < factors_.size(); ++j) {
      ...
      // LPK: Let's see what happened before the factor
      if (verbosity_ > 1) { 
        factor->Print(cout);
        cout << "dual_obj before" << dual_obj;
      }
  ...
  for (t = 0; t < ad3_max_iterations_; ++t) {
    ...
    for (int j = 0; j < factors_.size(); ++j) {
      ...
      for (int i = 0; i < factor_degree; ++i) {
        ...
        maps_[m] = variable_posteriors[i];
        //if (verbosity_ > 1) { 
        //  cout << "map[m]" << variable_posteriors[i] << "\n";
        //}
      }
    ...
    for (int i = 0; i < variables_.size(); ++i) {
      ...
      for (int j = 0; j < variable_degree; ++j) {
        ...
        //if (verbosity_ > 1) { 
        // cout << "diff_penalty = maps_[m] - maps_av_[i]\t" << diff_penalty << "\t" << maps_[m] << "\t" << maps_av_[i] << "\n";
        //}
    ...
    if (compute_dual) {
      ...
      for (int j = 0; j < factors_.size(); ++j) {
        ...
        dual_obj += val + delta;
        if (verbosity_ > 1) { 
        // LPK: Let's see what happened after the factor
        cout << "\t val = " << val << "\tdelta = " << delta << "\n";
        cout << "\t dual_obj after\t" << dual_obj << "\n\n";
        }
      }
